...
TRACKER_FILE='download_tracker_nsrp_obp.json'
TMP_DIR='tmp'

GITHUB_URL_PREFIX='https://github.com/NSRP'
GITHUB_API_URL_PREFIX='https://api.github.com/repos/NSRP'

# setup logging
logger = logging.getLogger('nsrp_obp_downloader')
...
def create_tracker_file():
    ...
    data = {
        'jenkins_trigger': {
            'jenkins_name': 'nsrp-obp-trigger-release',
            'last_successful_build_number': '0'
        },
        'jenkins_projects': [
            {
                'jenkins_name': 'NSRP-OBP',
                'last_successful_build_number': '0',
                'artifact-prefix': ''
            }
        ],
...
def check_trigger(tracker):
    ...
    last_build_url='%s/view/NSRP/job/%s/lastSuccessfulBuild/buildNumber' % (
...
def get_jenkins_artifacts(tracker):
    ...
    for i in tracker['jenkins_projects']:
        ...
        if last_build_number != i['last_successful_build_number']:
            ...
            r = requests.get(
                '%s/view/NSRP/job/%s/lastSuccessfulBuild/artifact/*zip*/archive.zip' % (
                    cfg.config['JENKINS_URL_PREFIX'], j_name),
